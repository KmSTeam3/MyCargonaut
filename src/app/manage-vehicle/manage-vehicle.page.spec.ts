...
describe('ManageVehiclePage', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      providers: [ ManageVehiclePage ,
        {provide: AuthService},
        {provide: VehicleService },
        {provide: AngularFirestore, useValue: environment.firebaseConfig}]
    }).compileComponents();
  ...
  it('should be empty', () => {
    ...
    expect(testComponent.listVehicle).toEqual([]);
  });
  ...
  it('should be no empty', function(done) {
    ...
    authService.login(value).then(() => {
      authService.checkAuthState().subscribe( (user) => {
        const vehicle: Vehicle = new Vehicle('test', 'asd', user.uid, 123, 123, 123, 123, 123);
        const testListVehicle: Vehicle[] = [vehicle];
        testComponent.renderList(user.uid);
        expect(testComponent.listVehicle).toEqual(testListVehicle);
      });
      done();
    });
  });
